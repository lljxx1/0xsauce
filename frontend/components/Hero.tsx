...
const Hero: FC<Props> = ({ fallback, collectionId }) => {
  ...
  const floor = collection.data?.collection?.floorPrice

  const statsObj = {
    count: Number(collection.data?.collection?.supply ?? 0),
    floor: floor ? parseFloat(floor) : 0,
    stolen: collection.data?.collection?.totalStolen,
  }
  ...
  const header = {
    ...
    image: collection?.data?.collection?.thumb as string,
