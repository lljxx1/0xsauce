...
const Home: NextPage<Props> = ({ fallback, id }) => {
  ...
  console.log( 'error', tokens.error, fallback.tokens);
        ...
        >
          ...
          <Tabs.Content value="items" asChild>
            <>
              ...
              <div className="col-span-full mx-6 mt-4 sm:col-end-[-1]">
...
export const getStaticProps: GetStaticProps<{
  collectionId?: string
  fallback: {
    collection: definitions['getCollection']
    tokens: definitions['getTokens']
  }
  id: string | undefined
}> = async ({ params }) => {
  ...
  // COLLECTION
  const collectionUrl = new URL('/api/collection', RESERVOIR_API_BASE)
  setParams(collectionUrl, {
    id
  })
  ...
  // TOKENS
  const tokensUrl = new URL('/api/tokens', RESERVOIR_API_BASE)
  setParams(tokensUrl, {
    collection: id
  })
